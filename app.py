"""
CB Application Form Stamper — v4
=================================
ROOT CAUSE OF SERIES RESET:
  Render's free tier has an EPHEMERAL filesystem.
  Every time the server sleeps and restarts, stamper.db is wiped,
  resetting the counter to CB500000001.

FIX IN THIS VERSION:
  The counter and all logs now live in a PERSISTENT PostgreSQL database
  (free on Supabase / Neon / Railway — survives server sleep, restart,
  redeploy, forever).

  DB layer auto-detects environment:
    • DATABASE_URL env var set  →  PostgreSQL  (production)
    • DATABASE_URL not set      →  SQLite       (local testing only)

  All other features (3 templates, all-page stamping, 9-digit CB series,
  MIS report, admin routes) are unchanged from v3.

SETUP:
  See the comment block "FIRST-TIME SETUP" below, or read DEPLOY_GUIDE.txt.
"""
...
#       • Copy the "URI" that starts with:  postgresql://postgres:...
#
#  2. Add it as an environment variable on Render:
#       Key:   DATABASE_URL
#       Value: postgresql://postgres:<password>@<host>:5432/postgres
#
#  3. Add psycopg2-binary to requirements.txt (already done in the new file)
#
#  4. Deploy. The DB tables are created automatically on first startup.
#     The counter starts at CB500000001 and NEVER resets again.
#
# ────────────────────────────────────────────────────────────────────────────
...
CB_START  = 500_000_000   # Series starts at CB500000001
...
NUM1_X, NUM1_Y = 370, 718
NUM2_X, NUM2_Y = 480, 718
...
if USE_POSTGRES:
    ...
    def _pg_conn():
        """
        Parse DATABASE_URL into individual components and pass them to
        psycopg2.connect() as keyword arguments.

        WHY: Passing the raw URL string to psycopg2 breaks when the password
        contains special characters like @  #  $  !  %  &  because the URL
        parser uses those characters as delimiters and misreads the host/port.
        Parsing the URL with urllib then passing fields separately bypasses
        the URL parser entirely for the password — it is treated as a plain
        string with no special meaning.

        unquote() handles passwords that Supabase has already percent-encoded
        (e.g. %40 → @) so they are passed to Postgres in their original form.
        """
        # Normalise the scheme — Render sometimes injects "postgres://"
